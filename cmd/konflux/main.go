...
func main() {
	...
	for _, configFile := range configFiles {
		...
		if err := cleanupAutogenerated(context.Background(), konfluxDir); err != nil {
...
func generateConfig(ctx context.Context, config k.Config, dir string, dryRun bool) error {
	...
	for _, repo := range config.Repos {
		...
		// Let's not remove GH workflow files
		//if err := cleanupAutogenerated(ctx, filepath.Join(checkoutDirMain, gitHubDir)); err != nil {
		//	return err
		//}
		...
		for _, branch := range repo.Branches {
			...
			for _, v := range branch.Versions {
				...
				if err := cleanupAutogenerated(ctx, filepath.Join(checkoutDir, ".tekton")); err != nil {
					return err
				}
				//Generate Tekton
				if err := generateTekton(version, filepath.Join(checkoutDir, ".tekton")); err != nil {
					log.Fatalln(err)
				}
				if repo.Upstream != "" {
...
func cleanupAutogenerated(ctx context.Context, dir string) error {
	...
	if out, err := run(ctx, dir, "bash", "-c", "grep -l -r '# Generated by openshift-pipelines/hack. DO NOT EDIT.' ."); err != nil {
