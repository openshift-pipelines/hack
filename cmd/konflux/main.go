...
func generateGitHubWorkflow(branch k.Branch, target string) error {
	...
	if !branch.Repository.Application.Version.Released {
...
func cleanupAutogenerated(ctx context.Context, application k.Application, dir string) error {
	...
	if out, err := run(ctx, dir, "grep", "-lr", header); err != nil {
		log.Printf("Couldn't grep for autogenerated content: %s, %s", err, string(out))
	} else {
